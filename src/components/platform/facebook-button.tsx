...
export function FacebookButton({
  ...
  const onClick = async () => {
    ...
    if (error) {
      ...
      if (/not enabled|unsupported provider/i.test(msg)) {
        toast.error("تسجيل الدخول بـ Facebook يتطلب تفعيل Provider في إعدادات Supabase مع App ID والـ Secret");
      } else {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-12 w-full items-center justify-center gap-3 rounded-xl border border-[#1877F2]/30 bg-[#1877F2]/10 text-sm font-bold text-zinc-100 transition-colors hover:border-[#1877F2]/60 hover:bg-[#1877F2]/20"
    >
