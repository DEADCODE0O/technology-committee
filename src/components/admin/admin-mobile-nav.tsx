...
export function AdminMobileNav({ items, userEmail, userRoleLabel }: AdminMobileNavProps) {
  ...
  return (
    <>
      {/* ── 1. زر فتح القائمة داخل الهيدر العلوي ── */}
      ...
      {/* ── 2. نقل الدرج والشريط السفلي إلى document.body عبر Portal لمنع انحباسه داخل backdrop-blur للهيدر ── */}
      {mounted &&
        createPortal(
          <>
            {/* خلفية التعتيم المعزولة بـ Portal */}
            <div
              className={`fixed inset-0 z-[9998] bg-black/75 backdrop-blur-xs transition-opacity duration-300 ${
            ...
            {/* لوح القائمة المنزلق بحرية مباشرة على document.body */}
            <aside
              className={`fixed top-0 bottom-0 right-0 z-[9999] flex h-full w-[85vw] max-w-[320px] flex-col bg-card border-s border-border shadow-2xl transition-transform duration-300 ease-in-out ${
            ...
            >
              ...
              {/* د) قائمة الأقسام المجمعة — تمرير حر وسلس 100% */}
              <nav
              ...
              {/* هـ) فوتر القائمة: الموقع العام وبيانات المشرف وتسجيل الخروج مع مراعاة منطقة الأمان */}
            ...
            {/* ── 3. شريط التنقل السفلي السريع للأدمن على الموبايل ── */}
            <nav
              className="fixed bottom-0 inset-x-0 z-[9990] flex h-16 items-center justify-around border-t border-border bg-card/95 backdrop-blur-xl lg:hidden px-2 shadow-2xl"
