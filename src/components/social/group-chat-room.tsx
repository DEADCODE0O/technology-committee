...
import { sendChatMessage, getChatRoomMessages } from "@/actions/chat";
...
export function GroupChatRoom({
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    const res = await sendChatMessage(room.id, clean);
    setIsSending(false);
  ...
  return (
    <div className="flex flex-col h-[82vh] rounded-3xl border border-border bg-card overflow-hidden shadow-xl">
        ...
        <div className="bg-gold/10 border-b border-gold/20 p-3 text-xs text-foreground flex items-start gap-2.5">
          ...
          <div className="flex-1 space-y-1">
            ...
            <ul className="list-disc list-inside text-[11px] text-muted-foreground space-y-0.5">
              ...
              <li>يمنع مشاركة أرقام الهواتف أو البيانات الحساسة على العام.</li>
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                {/* فقاعة الرسالة */}
                <div
                  className={`max-w-[80%] sm:max-w-[70%] rounded-2xl p-3 shadow-sm text-xs leading-5 break-words ${
                    isMe
                ...
                >
                  ...
                  {/* التوقيت */}
                  <div
                    className={`mt-1 flex items-center justify-end text-[9px] ${
      ...
      <form
        onSubmit={handleSend}
        className="border-t border-border p-3 sm:p-4 bg-muted/30"
      >
        <div className="relative flex items-end gap-2">
          <div className="relative flex-1">
            <textarea
              ...
              placeholder="اكتب رسالتك في الشات العام... (Enter للإرسال)"
              rows={1}
              maxLength={500}
